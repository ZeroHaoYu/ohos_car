...
bool SerialInterfaceLinux::Open(std::string &port_name, uint32_t com_baudrate) {
  ...
  if (-1 == com_handle_) {
    printf("Open open error,%s", strerror(errno));
    return false;
  }
  ...
  if (ioctl(com_handle_, _IOC(_IOC_READ, 'T', 0x2A, sizeof(struct asmtermios::termios2)), &options)) {
    printf("TCGETS2 first error,%s", strerror(errno));
    if (com_handle_ != -1) {
      close(com_handle_);
      com_handle_ = -1;
    }
  ...
  if (ioctl(com_handle_, _IOC(_IOC_WRITE, 'T', 0x2B, sizeof(struct asmtermios::termios2)), &options)) {
    printf("TCSETS2 error,%s", strerror(errno));
    if (com_handle_ != -1) {
      close(com_handle_);
      com_handle_ = -1;
    }
  ...
  if (ioctl(com_handle_, _IOC(_IOC_READ, 'T', 0x2A, sizeof(struct asmtermios::termios2)), &options)) {
    printf("TCGETS2 second error,%s", strerror(errno));
    if (com_handle_ != -1) {
      close(com_handle_);
      com_handle_ = -1;
    }
