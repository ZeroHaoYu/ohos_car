...
bool LDLidarDriver::Start(LDType product_name, 
  ...
  if (LDType::NO_VERSION == product_name) {
    printf("input <product_name> is abnormal.\r\n");
    return false;
  }

  if (serial_port_name.empty() || (0 == serial_baudrate)) {
    printf("input serial param error.\r\n");
    return false;
  }

  if (register_get_timestamp_handle_ == nullptr) {
    printf("get timestamp fuctional is not register.\r\n");
    return false;
  }
  ...
  if (COMM_SERIAL_MODE == comm_mode) {
    ...
    if (!(comm_serial_->Open(serial_port_name, serial_baudrate))) {
      printf("serial is not open:%s", serial_port_name.c_str());
      return false;
    }
  } else {
    printf("input <comm_mode> value is not \"ldlidar::COMM_SERIAL_MODE\"\r\n");
    return false;
  }
...
//     printf("input server_ip or server_port is null\r\n");
//     return false;
//   }

//   if ((COMM_NO_NULL == comm_mode) || (COMM_SERIAL_MODE == comm_mode)) {
//     printf("input comm_mode param is error.\r\n");
//     return false;
//   }
...
//       if (!result) {
//         printf("client host: create socket is fail.\r\n");
//         return false;
//       }
//       printf("client host: create socket is ok.\r\n");
//     }
//       break;
...
//       if (!result) {
//         printf("server host: create socket is fail.\r\n");
//         return false;
//       }
//       printf("server host: create socket is ok.\r\n");
//     }
//       break;
...
//         printf("client host: send request to server is fail. %s", strerror(errno));
//         return false;
//       }
//       printf("client host: create socket is ok.\r\n");
//     }
...
//       }
//       printf("server host: create socket is ok.\r\n");
//       printf("server host: wait client ack connect..\r\n");
...
bool LDLidarDriver::WaitLidarCommConnect(int64_t timeout) {
  ...
  do {
    if (comm_pkg_->GetLidarPowerOnCommStatus()) {
      is_recvflag = true;
      printf("lidar power on comm is ok.\r\n");
    }
    usleep(1000);
